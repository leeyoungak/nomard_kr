...
## - [x] Phase 1 — 기반 안정화: Mock Auth & 내비게이션 수정
...
**디자인**
- [ ] `app/login/page.tsx` — warm nature → 다크 터미널 테마로 전면 재작성 *(원복: 기존 디자인 유지)*
- [ ] `app/register/page.tsx` — warm nature → 다크 터미널 테마로 전면 재작성 *(원복: 기존 디자인 유지)*
...
- [x] `City` 인터페이스에 필드 추가: `slug`, `description`, `pros`, `cons`, `neighborhoods` (동네 배열), `coworkingSpots` (이름/주소/가격/wifi 속도), `cafes` (이름/주소/특징)
- [x] 기존 6개 도시에 위 확장 데이터 채우기

**도시 목록 페이지** (`app/cities/page.tsx`)
- [x] Server Component로 작성
- [x] 상단: 페이지 헤더 (`> /cities — 노마드 도시 탐색기`)
- [x] 필터 칩 (전체/가성비/인터넷/자연 등) — Client Component로 분리
- [x] 정렬 드롭다운 (종합점수/생활비/와이파이 속도)
- [x] CityCard 그리드 (홈과 동일 컴포넌트, `href="/cities/[slug]"` 추가)

**도시 상세 페이지** (`app/cities/[slug]/page.tsx`)
- [x] Server Component, `generateStaticParams`로 정적 생성
- [x] 헤더: 도시명, 지역, badgeLabel, 종합 점수
- [x] 스코어카드: 인터넷/생활비/카페/안전 4개 지표 바 차트 (CSS only)
- [x] 월 생활비 범위 표시
- [x] 추천 동네 섹션
- [x] 코워킹/카페 스팟 목록 (카드 형태)
- [x] 장점/단점 섹션
- [x] 해당 도시 리뷰 3개 표시
- [x] 뒤로가기 링크 (`← 도시 목록으로`)

**홈페이지 연결**
- [x] `components/home/CityCard.tsx` — 카드 클릭 시 `/cities/[slug]`로 이동
- [x] `components/home/FilterAndCityGrid.tsx` — CityCard에 slug prop 전달
...
**Mock 데이터 확장** (`lib/mock-data.ts`)
- [x] `Meetup` 인터페이스에 필드 추가: `slug`, `citySlug`, `description`, `organizer`, `status` (`open`/`full`/`ended`)
- [x] 밋업 데이터를 4개 → 10개 이상으로 확장 (다양한 도시, 태그, status 포함)

**밋업 목록 페이지** (`app/meetups/page.tsx`)
- [x] 상단 헤더 + 통계 (이달 밋업 수, 총 참가자 수)
- [x] 도시 필터 탭 (전체/서울/제주/부산 등)
- [x] 태그 필터 칩
- [x] `MeetupCard` 컴포넌트 (`components/meetups/MeetupCard.tsx`)
  - [x] 제목, 날짜, 장소, 태그, RSVP 현황 바
  - [x] `open`: 신청 가능 버튼 (초록), `full`: 마감 뱃지 (빨강), `ended`: 종료 뱃지 (회색)
- [x] RSVP 버튼 클릭 → 토스트 메시지 (`신청이 완료되었습니다.`) — 실제 저장 없음

**홈페이지 연결**
- [x] `components/home/MeetupsSection.tsx` — "더 보기" 링크 → `/meetups`

### 검증/확인
- [x] `/meetups` 접속 시 10개 이상 밋업 카드 렌더링 확인
- [x] 도시 탭 클릭 시 해당 도시 밋업만 표시 확인
- [x] 태그 필터 동작 확인
- [x] `open` 상태 RSVP 버튼 클릭 시 토스트 메시지 확인
- [x] `full` 상태 마감 뱃지 표시 확인
- [x] 홈 MeetupsSection "더 보기" 링크 → `/meetups` 이동 확인
- [x] 모바일 레이아웃 확인

---

## - [x] Phase 4 — 커뮤니티: 리뷰 & 팁 게시판
...
**Mock 데이터 확장** (`lib/mock-data.ts`)
- [x] `Review` 인터페이스에 필드 추가: `id`, `citySlug`, `likes`, `type` (`review`/`tip`)
- [x] 리뷰 데이터를 3개 → 15개 이상으로 확장 (모든 도시 커버, `tip` 타입 포함)

**커뮤니티 페이지** (`app/community/page.tsx`)
- [x] 탭 UI: `리뷰` / `팁 & 정보` — Client Component
- [x] 도시 필터 드롭다운
- [x] 정렬: 최신순 / 좋아요순 / 평점순
- [x] `ReviewCard` 컴포넌트 (`components/community/ReviewCard.tsx`)
  - [x] userId, 도시, 구, 날짜, 별점, 본문, 좋아요 버튼 (UI only)
- [x] 리뷰 작성 버튼 → `WriteReviewModal` 열기
- [x] `WriteReviewModal` (`components/community/WriteReviewModal.tsx`)
  - [x] 도시 선택, 구/동 입력, 별점 클릭 선택, 본문 textarea
  - [x] 제출 시 토스트 메시지 (`리뷰가 등록되었습니다.`) — 실제 저장 없음

### 검증/확인
- [x] `/community` 접속 시 15개 이상 리뷰 카드 렌더링 확인
- [x] 리뷰/팁 탭 전환 동작 확인
- [x] 도시 필터 드롭다운 동작 확인
- [x] 좋아요 버튼 클릭 시 시각적 피드백 (토글) 확인
- [x] 리뷰 작성 모달 열기/닫기 확인
- [x] 리뷰 작성 폼 제출 시 토스트 메시지 확인
- [x] 모바일 레이아웃 확인
...
- [x] 전체 통계 요약 섹션 (StatsBar와 동일 데이터, 상세 설명 추가)
- [x] 도시 종합 순위 테이블 (rank, 도시명, 점수, 인터넷/생활비/카페/안전 4개 지표)
- [x] 지표별 바 차트 (CSS `width` 퍼센트 기반, 애니메이션 포함)
  - [x] 인터넷 속도 랭킹
  - [x] 생활비 효율 랭킹
  - [x] 카페 접근성 랭킹
  - [x] 안전도 랭킹
- [x] 도시 비교 선택 UI: 2개 도시를 선택해 나란히 비교 — Client Component
- [x] 월 생활비 비교 (min~max 범위 바)

### 검증/확인
- [x] `/stats` 접속 시 모든 섹션 렌더링 확인
- [x] 4개 지표 바 차트에서 점수가 높은 도시가 더 긴 바로 표시되는지 확인
- [x] 도시 비교 UI에서 2개 도시 선택 후 나란히 비교 렌더링 확인
- [x] 생활비 범위 바가 min~max 올바르게 표시 확인
- [x] Server Component로 작성되어 JS 인터랙션 없는 정적 섹션은 hydration 없음 확인
- [x] 모바일 레이아웃 확인
